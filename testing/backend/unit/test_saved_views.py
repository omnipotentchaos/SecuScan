...
async def test_unauthenticated_request_rejected(app_client: AsyncClient):
    ...
    client_app = getattr(app_client, "app", None)
    old_override = client_app.dependency_overrides.pop(require_api_key, None) if client_app else None
    try:
        ...
        if client_app and old_override:
            client_app.dependency_overrides[require_api_key] = old_override
